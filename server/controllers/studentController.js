...
const getAllStudents = async (req,res)=>{
    try{
    const studentsList = await Student.find().populate("class1",{grade:1,number:1}).sort({grade:1,number:1,name:1})
    if(!studentsList)
...
const createStudent = async (req,res)=>{
    ...
    const dup = await Student.findOne({name,class1}).lean()
    if(dup)
        return res.status(409).json({msg:'קיימת כבר תלמידה בשם זה'})
    const dup2 = await Student.findOne({idNum}).lean()
    if(dup2)
    ...
    const student = await Student.create({idNum,name,class1,comment})
    if(!student)
...
const deleteStudent = async (req,res)=>{
    ...
    const delLate =await Late.deleteMany({student:id})
    const del = await student.deleteOne()
...
const deleteGrade8 = async (req,res)=>{
    try{
    var msg =''
    const classes = await Class.find({grade:'ח'})
    if(!classes)
        return res.status(401).json({msg:'לא נמצאו תלמידות'})
    classes.forEach(async c=>{
        const del = await Student.deleteMany({class1:c._id})
        msg+=`כיתה ח${c.number}: נמחקו ${del.deletedCount} ,תלמידות`
    })
    const del = await Class.deleteMany({grade:'ח'})
